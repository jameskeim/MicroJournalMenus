...
count_file() {
  ...
  if [ -f "$file" ]; then
    ...
    # Show word count method used
    if command -v pandoc >/dev/null 2>&1 && [[ "$file" == *.md ]]; then
      echo "Words: $words (accurate) | Characters: $chars | Lines: $lines | Paragraphs: $paragraphs | $read_str read"
    else
...
# Function to count words for today (columnar format with goal integration)
count_today() {
  load_goals
